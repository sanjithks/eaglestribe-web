...
// Fetch a ride from Strapi using the documentId (slug)
...
export async function generateMetadata({ params }) {
  ...
  return {
    title: ride?.attributes?.title || "Ride Details",
    description: ride?.attributes?.short_description || "A ride from Eagles Tribe MC.",
  };
}
...
export default async function RideDetailPage({ params }) {
  ...
  // Get full image URL for banner
  const bannerUrl = featured_image?.data?.attributes?.url || null;

  // Ride gallery is in ride_gallery.data as array of image objects
      ...
      {bannerUrl && (
        <div className="relative my-6 max-w-5xl mx-auto">
          <Image
            ...
            width={featured_image.data.attributes.width || 1200}
            height={featured_image.data.attributes.height || 675}
        ...
        <article className="prose dark:prose-invert max-w-none">
          <div
            dangerouslySetInnerHTML={{
              __html: detailed_write_up?.replace(/\n/g, "<br />"),
            }}
          />
        </article>
